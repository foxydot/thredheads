...
add_action( 'plugins_loaded', 'pb_updater_register', 50 );
if ( !function_exists( 'pb_updater_register' ) ) {
	function pb_updater_register() {
		$GLOBALS['pb_classes_upgrade_registration_list'][$this->_var]['version'] = '1.0.0';
	}
}
...
class iThemesPluginUpgrade {
			...
			private function get_plugin_options() {
				...
				//Get plugin options
				if ( is_multisite() ) {
					$options = get_site_option( 'pluginbuddy_plugins' );
				} else {
					$options = get_option( 'pluginbuddy_plugins' );
				}

				$options = maybe_unserialize( base64_decode( $options ) );
			...
			private function save_plugin_options() {
				...
				$options = base64_encode( serialize( $this->plugins ) );
				//Get plugin options
				if ( is_multisite() ) {
			...
			private function get_defaults() {
				...
				$plugin_options->id = "0";
				$plugin_options->key = '';
				$plugin_options->key_status = 'invalid';
				$plugin_options->userhash = '';
				$plugin_options->username = '';
			...
			public function perform_remote_request( $args ) {
				...
				$body = wp_parse_args( $body, array( 
					...
					'userhash' => $this->plugins[ $this->plugin_slug ]->userhash,
					'username' => $this->plugins[ $this->plugin_slug ]->username,
					'action' => $action,
			...
			public function plugin_links($val) {
				...
				$val[sizeof($val)] = sprintf( "<a href='%s' class='thickbox'><img src='%s' style='vertical-align: -3px' /> Licenses</a>", $ajax_url, $this->plugin_url . '/lib/updater/key.png' );				
				return $val;
			} //end plugin_ilnks
			...
			public  function plugin_row( $plugin_name ){
				...
				<span style='border-right: 1px solid #DFDFDF; margin-right: 5px;'>
					<a class='thickbox' href='<?php echo esc_url( add_query_arg( array( 'slug' => $this->plugin_slug, 'action' => $this->plugin_slug . 'licenses', '_ajax_nonce' => wp_create_nonce( $this->plugin_slug . 'licenses' ), 'TB_iframe' => true ), admin_url( 'admin-ajax.php' ) ) ); ?>'><img src='<?php echo esc_url( $this->plugin_url . '/lib/updater/key.png' ); ?>' style='vertical-align: -3px;' /> Manage Licenses</a>
				</span>
...
$args = array(
	...
	'method' => 'POST',
	'upgrade_action' => 'check' );
$ithemes_upgrader = new iThemesPluginUpgrade( $args );
?>